...
goal_distance = h2(xNode, yNode, xTarget, yTarget); % cost h(n): heuristic cost of n
...
while((xNode ~= xTarget || yNode ~= yTarget) && NoPath == 1) % 目前坐标不等于Target 并且 there exists a path 继续search
    ...
    exp = expand_h2(xNode, yNode, path_cost, xTarget, yTarget, OBSTACLE, MAX_X, MAX_Y);
    ...
    for i = 1 : exp_count
        ...
        % A*
        for j = 1 : QUEUE_COUNT
            if(exp(i, 1) == QUEUE(j, 2) && exp(i, 2) == QUEUE(j, 3)) % check if nodes in exp is already in QUEUE
                QUEUE(j, 8) = min(QUEUE(j, 8), exp(i, 5));
                if QUEUE(j, 8) == exp(i, 5) % check if min(QUEUE(j, 8), exp(i, 5)) is equal to exp(i, 5)
