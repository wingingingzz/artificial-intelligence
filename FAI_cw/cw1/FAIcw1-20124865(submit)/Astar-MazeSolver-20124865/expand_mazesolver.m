...
function exp_array = expand_mazesolver(node_x, node_y, gn, xTarget, yTarget, OBSTACLE, MAX_X, MAX_Y)
    ...
    for k = 1 : -1 : -1 % explore surrounding locations
        for j = 1 : -1 : -1
            if (k ~= j && k ~= -j)  % change
                s_x = node_x + k;
                s_y = node_y + j;
                if( (s_x > 1 && s_x <= MAX_X - 1) && (s_y > 1 && s_y <= MAX_Y - 1)) % node within array bound % change
                    flag = 1;
                    for c1 = 1 : c2
