...
[MAX_X, MAX_Y] = size(maze); % change
OBSTACLE = [];
k = 1;
for i = 1 : MAX_X
    for j = 1 : MAX_Y
        if(maze(i, j) == 0) % change
            OBSTACLE(k, 1) = i;
            OBSTACLE(k, 2) = j;
            k = k + 1;
        end
        if(maze(i, j) == 3) % change
            xStart = i - 1;
            yStart = j;
        end
        if(maze(i, j) == 4) % change
            xTarget = i + 1;
            yTarget = j;
        end
    end
...
while((xNode ~= xTarget || yNode ~= yTarget) && NoPath == 1)
    ...
    exp = expand_mazesolver(xNode, yNode, path_cost, xTarget, yTarget, OBSTACLE, MAX_X, MAX_Y);
